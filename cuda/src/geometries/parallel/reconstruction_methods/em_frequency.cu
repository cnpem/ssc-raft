...
extern "C"{
  ...
  void emfreqgpu(float *count, float *recon, float *angles, float *flat,
    ...
    int niter, int gpu)
  {
      size_t blocksize_aux = calc_blocksize(
          blockgpu, nangles, nrays, zpad, true); // 32ul;
      ...
      for (size_t b = 0; b < blockgpu; b += blocksize) {
          ...
          emfreq(
              count + (size_t)b*nrays*nangles, recon + (size_t)b*nrays*nrays,
              angles, flat + (size_t)b*nrays,
              nrays, nangles, blocksize, zpad, interpolation, dx, tv_param, niter, gpu);
      }
    ...
    int niter, int ngpus, int* gpus)
  {
      ...
      for(t = 0; t < ngpus; t++){ 
          ...
          threads.push_back(std::async( std::launch::async, emfreqgpu, 
              count + (size_t)t * blockgpu * nrays * nangles, 
              recon + (size_t)t * blockgpu * nrays * nrays, 
              angles, flat + (size_t)t * blockgpu * nrays, 
              nrays, nangles, blockgpu,  zpad, interpolation, dx, tv_param, niter, gpus[t]));
      }
