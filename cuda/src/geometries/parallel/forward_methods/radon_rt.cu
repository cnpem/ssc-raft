...
extern "C"{
    ...
    __global__ void KRadon_RT(float* restrict frames, const float* image, int nrays, int nangles)
    {
        ...
        float btheta = float(M_PI)*float(ang + (float)nangles/2.0f)/nangles;
        
        float ct = (float)nrays/2.0f;
        float cos_t = cosf(btheta + 1E-5f);
        float sin_t = sinf(btheta + 1E-5f);

        float x = ct - 2.0f*ct*cos_t + (ray - (float)nrays/2.0f)*sin_t;
        float y = ct - 2.0f*ct*sin_t - (ray - (float)nrays/2.0f)*cos_t;
