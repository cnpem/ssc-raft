...
struct Image2D{
	...
	Image2D(size_t _sizex, size_t _sizey, size_t _sizez = 1, MemoryType memtype = MemoryType::EAllocManaged) : sizex(_sizex), sizey(_sizey), sizez(_sizez), size(_sizex * _sizey * _sizez), memorytype(memtype), gpuptr(nullptr), cpuptr(nullptr)
	{
	...
	Image2D(Type *newdata, size_t _sizex, size_t _sizey, size_t _sizez = 1, MemoryType memtype = MemoryType::EAllocManaged) : Image2D<Type>(_sizex, _sizey, _sizez, memtype)
	{
	...
	explicit Image2D(Image2D<Type> &other) : Image2D(other.sizex, other.sizey, other.sizez, (other.memorytype != MemoryType::ENoAlloc) ? other.memorytype : MemoryType::EAllocManaged)
	{
		CopyFrom(other);
	}
	...
	void SetGPUToZero()
	{
		HANDLE_ERROR(cudaMemset(gpuptr, 0, size * sizeof(Type)));
		SyncDebug;
	}
	void SetGPUToZero(cudaStream_t stream)
	{
		HANDLE_ERROR(cudaMemsetAsync(gpuptr, 0, size * sizeof(Type), stream));
		SyncDebug;
	}
	void SetCPUToZero()
	{
	...
	void AllocGPU()
	{
		if (!gpuptr && bHasAllocGPU())
			HANDLE_ERROR(cudaMalloc((void **)&gpuptr, sizeof(Type) * size));
	}
	void DeallocGPU()
	{
		if (gpuptr && bHasAllocGPU())
	...
	{
		if (cpuptr && bHasAllocCPU())
			delete[] cpuptr; // cudaFreeHost(cpuptr);
		cpuptr = nullptr;
	}
	void AllocManaged()
	{
...
struct MImage : public MultiGPU
{
	...
	MImage(size_t _sizex, size_t _sizey, size_t _sizez, bool _bBroadcast, const std::vector<int> &gpus, MemoryType memtype = MemoryType::EAllocManaged) : MultiGPU(gpus), sizex(_sizex), sizey(_sizey), sizez(_sizez), size(_sizex * _sizey * _sizez), bBroadcast(_bBroadcast), memorytype(memtype)
	{
	...
	MImage(dim3 dim, bool _bBroadcast, const std::vector<int> &gpus, MemoryType memtype = MemoryType::EAllocManaged) : MImage<Type>(dim.x, dim.y, dim.z, _bBroadcast, gpus, memtype){};
	MImage(Type *newdata, dim3 dim, bool _bBroadcast, const std::vector<int> &gpus, MemoryType memtype = MemoryType::EAllocManaged) : MImage<Type>(newdata, dim.x, dim.y, dim.z, _bBroadcast, gpus, memtype){};

	MImage(MImage<Type> &other) : MImage<Type>(other.sizex, other.sizey, other.sizez, other.bBroadcast, other.gpuindices, other.memorytype)
	{
		CopyFrom(other);
	};
...
extern "C"
{
	...
	void removeNan(float *sino, int length);
	__global__ void removeNan_block(float *blockSino, int nviews, int nrays, int blockSize);
