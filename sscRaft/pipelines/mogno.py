...
from ..processing.alignment.centersino import *
...
def Read_TomoFlatDark(dic):
    ...
    try:
        ...
        logger.error(f'Cannot find data on file {filepath}.')
        logger.error(f'Finishing run...')
        raise ValueError(f'Cannot find data on file {filepath}.')
...
def reconstruction_mogno(param = None):
    ...
    if param == None:
        logger.error(f'Missing argument of \'Read_Json()\'.')
...
def reconstruction_mogno_cli(dic:dict):
    ...
    tomogram,flat_,dark_ = Read_TomoFlatDark(dic)
    ...
    if dic['norm']:
        ...
        try:
            ...
            logger.error(f'Please guarantee that the data has entries: (angles,slices,rays).')
            logger.error(f'Finishing run...')
            sys.exit(1)
