...
def _iterations_em_mpfsupy_(sino, niter, device):
    ...
    for m in range(niter):
        _s_ = radon.radon_gpu( img_em, nangles, device)
...
def _build_em_mpfsupy_(params):
    ...
    b = int( numpy.ceil( nimages/ngpus )  ) 
...
def em( tomogram, dic ):
    ...
    * ``dic['niterations']``:  Number of iterations for the EM algorithm.
    ...
    _build_em_multiprocessing_reconstruction_ ( _params_ )

    sa.delete(name)
    
    return output_blocksino
