...
def phase_filters2(tomogram,dic):

    print('phase filters 2')
    ...
    out      = np.ones((nangles, nslices + 2 * pady, nrays + 2 * padx), dtype = np.float32)
    out = np.ascontiguousarray(out.astype(np.float32))
    outptr = out.ctypes.data_as(void_p)

    logger.info(f'tomogram shape = {tomogram.shape}.')
    logger.info(f'out shape = {out.shape}.')

    libraft.phase_filters2(outptr, tomogramptr, float_paramsptr, int_paramptr, int32(nrays), int32(nangles), int32(nslices), gpusptr, int32(ngpus))
