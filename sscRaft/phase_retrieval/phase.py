...
def phase_retrieval(frames, dic):
    ...
    tomo_dim        = dimension((nrays, nangles, nslices), (padding, padding, 0), blocksize = blocksize, padd_mode = padd_mode)
...
def paganin(frames, dic):
    ...
    required = ('required',)
    optional = ('gpu','beta/delta','padding','blocksize','z2[m]','energy[eV]','magn','detectorPixel[m]')
    default  = (  [0],         0.0,     0.25,          0,    1.0,         1.0,   1.0,               1.0)
    ...
    method          = ContrastFilterNumber('paganin')
    ...
    Params          = contrast_param(method, beta_delta, regularization = 0.0, post_process = 1)
