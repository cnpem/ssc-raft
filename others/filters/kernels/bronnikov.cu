...
extern "C" {
    ...
    {
        bronnikovPrep<<<gpus.Grd,gpus.BT>>>(projections, size);
 	...
 	__global__ void bronnikovKernel(GEO geometry, float *kernel, dim3 size, int phase_reg)
    {
		...
		float kernelX = 2.0 * float(M_PI) * geometry.z2x * geometry.lambda * wxx  / ( geometry.magnitude_x );
		float kernelY = 2.0 * float(M_PI) * geometry.z2y * geometry.lambda * wyy  / ( geometry.magnitude_y );

        kernel[ind] = 1.0 / ( phase_reg +  kernelX + kernelY );
