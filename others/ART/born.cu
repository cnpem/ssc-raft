...
extern "C" {
	...
	void _born_gpu(PAR param, float *projections, size_t nrays, size_t nangles, size_t nslices)
    {
        ...
        bornPrep<<<param.Grd,param.BT>>>(d_sino, param, nrays, nslices, nangles);

        bornKernel<<<param.Grd,param.BT>>>(kernel, param, param.Npadx, param.Npady, nangles);
 	...
 	__global__ void bornKernel(float *kernel, PAR param, size_t sizex, size_t sizey, size_t sizez)
    {
		...
		float kernelX = float(M_PI) * param.z2x * param.lambda * wxx  / ( magnx );
		float kernelY = float(M_PI) * param.z2y * param.lambda * wyy  / ( magny );

        kernel[ind] = 1.0 / ( param.alpha * cosf( kernelX + kernelY ) + sinf( kernelX + kernelY ) );
